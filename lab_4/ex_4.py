...
#Подключаемся к postgresql, базе lab_four
conn = psycopg2.connect(
    "host=localhost dbname=lab_four port=5432 user=postgres password=11111")
...
def create(conn):
    cur = conn.cursor()
    try:
        # Используем execute, для выполнение выражения SQL 
        ...
        # Создание таблицы с оценками сотрудников
        cur.execute(
        ...
        #Наполняем таблицу с информацией о работниках 
        cur.execute(
            "INSERT INTO employees (full_name, started, job_title, level, salary, department_name, permission) VALUES\
            ('Lewis Long', '2022-05-01', 'Project Manager', 'Middle', 2500, 'Management Department', True), \
            ('Tracy Banks', '2001-05-01', 'Fullstack Developer', 'Senior', 7500, 'IT Department', True), \
            ('Horace Flowers', '2021-05-01', 'Chief Marketer', 'Middle', 4000, 'Marketing Department', False), \
            ('Alexandrina Owen', '2004-01-10', 'Human Resources', 'Senior', 4000, 'HR Department', False), \
            ('Lawrence Butler', '2019-09-10', 'Buisness Analyst', 'Middle', 1500, 'Management Department', True), \
            ('Emery Sullivan', '2022-08-25', 'Quality Assure', 'Junior', 900, 'IT Department', False), \
            ('Malcolm Norman', '2019-06-11', 'Recruiter', 'Junior', 800, 'HR department', False), \
            ('Bryce Parrish', '2017-07-18', 'Frontend Developer', 'Middle', 1500, 'IT Department', True),  \
            ('Amos Arnold', '2022-07-15', 'Backend Developer', 'Junior', 1500, 'IT Department', True)"
        )
        conn.commit() 

        # Наполняем таблицу с отделами, название отдела,
        cur.execute(
            "INSERT INTO departments (department_name, head_of_department_full_name, employee_quantity) VALUES \
            ('Management Department', 'Lewis Long', 2), ('IT Department', 'Tracy Banks', 4), \
            ('Marketing Department', 'Horace Flowers', 1), ('HR Department', 'Alexandrina Owen', 2)"
        )
        conn.commit()
        cur.execute(
            "INSERT INTO employee_ratings (employee_id, working_quarter, evaluation) VALUES \
            (1, 2, 'A'), (1, 3, 'A'), (1, 4, 'A'), (1, 1, 'B'), (1, 2, 'A'), (1, 3, 'B'), \
            (2, 2, 'B'), (2, 3, 'A'), (2, 4, 'B'), (2, 1, 'A'), (2, 2, 'C'), (2, 3, 'A'), \
            (3, 2, 'C'), (3, 3, 'B'), (3, 4, 'B'), (3, 1, 'B'), (3, 2, 'B'), (3, 3, 'A'), \
            (4, 2, 'A'), (4, 3, 'A'), (4, 4, 'A'), (4, 1, 'B'), (4, 2, 'A'), (4, 3, 'C'), \
            (5, 2, 'B'), (5, 3, 'C'), (5, 4, 'B'), (5, 1, 'A'), (5, 2, 'B'), (5, 3, 'C'), \
            (6, 2, 'B'), (6, 3, 'A'), (6, 4, 'C'), (6, 1, 'A'), (6, 2, 'B'), (6, 3, 'B'), \
            (7, 2, 'D'), (7, 3, 'B'), (7, 4, 'A'), (7, 1, 'A'), (7, 2, 'B'), (7, 3, 'B'), \
            (8, 3, 'B'), (8, 4, 'B'), (8, 1, 'A'), (8, 2, 'A'), (8, 3, 'A'), \
...
# Добавляем в таблицу employees новых сотрудников
def add_department(conn):
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO employees (full_name, started, job_title, level, salary, department_name, permission) VALUES\
        ('Daniel Williams', '2023-09-20', 'Data Scientist', 'Middle', 2500, 'Data Mining Department', True), \
        ('Anton Green', '2023-09-25', 'Data Scientist', 'Junior', 1200, 'Data Mining Department', False), \
        ('Timothy Hicks', '2023-09-05', 'Data Analyst', 'Senior', 4000, 'Data Mining Department', True)"
    )
    conn.commit() 
    cur.execute(
        "INSERT INTO departments (department_name, head_of_department_full_name, employee_quantity) VALUES\
        ('Data Mining Department', 'Timothy Hicks', 2)"
    )
    conn.commit() 

# Вывод уникальных номеров сотрудников, с ФИО и стаж работы – для всех сотрудников компании
...
def get_employees_with_d_e(conn):
    ...
    cur = conn.cursor()
    cur.execute("SELECT e.id, rat.evaluation \
                FROM employees AS e FULL JOIN employee_ratings AS rat \
                ON e.id = rat.employee_id \
                WHERE rat.evaluation = 'D' OR rat.evaluation = 'E'")
    print(tabulate(cur.fetchall()))
...
def get_aver_exp_for_level(conn):
    ...
    date = datetime.date.today()
    cur = conn.cursor()
    cur.execute(f"SELECT level, AVG((CAST('{date}' AS date) - CAST(started AS date))/365) AS experience \
                FROM employees \
                GROUP BY level")
    print(tabulate(cur.fetchall()))
